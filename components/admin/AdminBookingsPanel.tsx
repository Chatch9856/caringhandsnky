...
import LoadingSpinner from '../LoadingSpinner'; // Keep for main loading
import SkeletonLoader from '../shared/SkeletonLoader'; // For card skeletons
...
import SegmentedControl, { SegmentedControlOption } from '../shared/SegmentedControl';
...
const AdminBookingsPanel: React.FC = () => {
  ...
  return (
    <div className="space-y-6"> {/* Outer container for the panel */}
      ...
      {activeSubTab === BOOKINGS_SUB_TAB_LIST_VIEW && (
        <>
          ...
          {isLoadingBookings && !bookings.length ? ( // Show skeletons only if no data is present yet
...
interface BookingSectionProps {
  ...
  defaultOpen?: boolean;
  isLoading: boolean; // To show skeleton for cards
}
...
const BookingSection: React.FC<BookingSectionProps> = ({ title, bookings, onUpdateStatus, onViewDetails, defaultOpen = true, isLoading }) => {
  ...
  if (isLoading && !bookings.length) { // Show skeleton if loading and no bookings for this section yet
    return (
      <div className="mb-6">
        <div className="p-4 bg-white rounded-2xl shadow-lg border border-slate-200">
            <div className="bg-slate-200 h-8 w-1/2 rounded mb-4 animate-pulse"></div> {/* Title skeleton */}
